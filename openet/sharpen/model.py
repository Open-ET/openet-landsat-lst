...
class Model:
    ...
    # TODO: Decide if this shoudl be a lazy property since it doesn't have inputs
    # @lazy_property
    def thermal(self):
        ...
        inputs = self.image.select(bands) \
        ...
        tir_org_agg = self.image.select(['tir']).pow(4) \
            .reduceResolution(ee.Reducer.mean()) \
            .reproject(crs=crs, crsTransform=ec_transform).pow(0.25)
