...
def thermalSharpening(image):
  ...
  """
    ...
    tir_sp_local = inputs.multiply(coefficients).reduce(ee.Reducer.sum()).pow(
        0.25)
...
def main(argv):
    ...
    for i in range(n):
        ...
        sensor_dict = {'LANDSAT_5': 'LT05', 'LANDSAT_7': 'LE07',
                       'LANDSAT_8': 'LC08'}
        ...
        scale = ee.Projection(
            landsat_image.select([0]).projection()).nominalScale().getInfo()
    ...
    """
        ...
        task = ee.batch.Export.image.toAsset(image=tir_sp_image,
                                             ...
                                             # scale=scale)
                                             crsTransform=str(
                                                 transform))  # the input has to be a string '[a,b,c]'
